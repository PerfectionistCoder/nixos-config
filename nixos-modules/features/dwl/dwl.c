...
void applyminimalborders(Client *c, Monitor *m) {
...
Vector position_of_box(const struct wlr_box *box) {
  return (Vector){
      .x = box->x + box->width / 2,
...
Vector diff_of_vectors(Vector *a, Vector *b) {
  return (Vector){
      .x = b->x - a->x,
      .y = b->y - a->y,
  };
...
const char *direction_of_vector(Vector *vector) {
...
Client *client_in_direction(const char *direction, const int *skipfloat) {
  ...
  if (!cfocused || cfocused->isfullscreen ||
      (skipfloat && cfocused->isfloating))
    return NULL;
...
void wl_list_swap(struct wl_list *list1, struct wl_list *list2) {
...
int modekeybinding(uint32_t mods, xkb_keysym_t sym) {
  int handled = 0;
  const Modekey *mk;
  const Key *k;
...
void printstatus(void) {
  ...
  wl_list_for_each(m, &mons, link) {
    ...
    printf("%s mode %s\n", m->wlr_output->name,
           modes_labels[active_mode_index] ? modes_labels[active_mode_index]
                                           : "");
  }
  fflush(stdout);
}
...
void resizenoapply(Client *c, struct wlr_box geo, int interact) {
  c->geom = geo;
  c->interact = interact;
}
